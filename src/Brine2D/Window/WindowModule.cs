...
public unsafe sealed class WindowModule : Module
{
    ...
    // 
     	...
     	f.minwidth = SysMath.Max(f.minwidth, 1);
     	f.minheight = SysMath.Max(f.minheight, 1);
     
     	SDLDisplayIDs displays = new();
     	int displaycount = displays.count;
     
     	f.displayindex = SysMath.Min(SysMath.Max(f.displayindex, 0), displaycount - 1);
...
public (double width, double height) GetDesktopDimensions(double displayindex = 1) =>
    ...
    /// </summary>
    /// <returns>True if the window has the focus or false if not.
    /// </returns>
    public bool HasFocus()
    {
        return window != null && SDL_GetKeyboardFocus() == window;
    }

    /// <summary>
    /// Checks if the game window has mouse focus.
    /// </summary>
    /// <returns>True if the window has mouse focus or false if not.
    /// </returns>
    public bool HasMouseFocus()
    {
        return window != null && SDL_GetMouseFocus() == window;
    }

    /// <summary>
    /// Checks if the window has been created.
    /// </summary>
    /// <returns>True if the window has been created, false otherwise.
    /// </returns>
    ...
    /// </summary>
    /// <returns>True if system display sleep is enabled / allowed, false otherwise.
    /// </returns>
    public bool IsDisplaySleepEnabled()
    {
    ...
    /// </summary>
    /// <returns>True if the window is currently maximized in windowed mode, false otherwise.
    /// </returns>
    public bool IsMaximized()
    {
        return window != null && SDL_GetWindowFlags(window).HasFlag(SDL_WindowFlags.SDL_WINDOW_MAXIMIZED);
    }
    ...
    {
        return window != null && SDL_GetWindowFlags(window).HasFlag(SDL_WindowFlags.SDL_WINDOW_MINIMIZED);
    }
    ...
    /// </summary>
    /// <returns>True if the window is visible or false if not.
    /// </returns>
    public bool IsVisible()
    {
        ...
        return !flags.HasFlag(SDL_WindowFlags.SDL_WINDOW_HIDDEN)
               && !flags.HasFlag(SDL_WindowFlags.SDL_WINDOW_MINIMIZED);
    }
    ...
    /// <returns>True if an attempt to enter fullscreen was successful, false otherwise.
    /// </returns>
    ...
    /// <returns>Whether the icon has been set successfully.
    /// </returns>
    ...
    public bool SetMode(int width, int height, WindowSettings flags)
    {
        ...
        return SetWindow(width, height, flags);
    }
    ...
    public void SetPosition(int x, int y, int displayindex = 1)
    {
        ...
        if (window == null)
         ...
         displayindex = SysMath.Min(SysMath.Max(displayindex, 0), displayids.count - 1);
    ...
    /// <returns>The converted number, in pixels.
    /// </returns>
