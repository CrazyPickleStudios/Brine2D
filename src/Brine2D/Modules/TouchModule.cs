...
public sealed class TouchModule : Module
{
    ...
    ///     </list>
    /// </returns>
    public (double x, double y) GetPosition(IntPtr id)
    {
    ...
    ///     1.
    /// </returns>
    public double GetPressure(IntPtr id)
    {
    ...
    /// </returns>
    public IEnumerable<IntPtr> GetTouches()
    {
        for (var i = 0; i < _touches.Count; i++)
        {
            // TODO: Verify that this cast actually works.
            yield return (IntPtr)_touches[i].id;
        }
    }

    private TouchInfo GetTouch(IntPtr id)
    {
        for (var i = 0; i < _touches.Count; i++)
        {
            var touch = _touches[i];
            if ((IntPtr)touch.id == id)
                return touch;
        }
